...
const ineqLow = (): Problem => {
  ...
  if (type===1) return createProblem('2-1-4', diff, `$x < 2$일 때, 식 $x+3$의 범위를 부등식으로 나타내시오.`, `x+3 < 5`);
  if (type===2) return createProblem('2-1-4', diff, `일차부등식 $2x > 6$의 해를 구하시오.`, `$x > 3$`);
  if (type===3) return createProblem('2-1-4', diff, `일차부등식 $-x \\le -3$의 해를 구하시오.`, `$x \\ge 3$`);
...
const ineqHigh = (): Problem => {
  ...
  if (type===1) return createProblem('2-1-4', diff, `어떤 물건의 원가는 4000원이다. 이 물건을 정가의 20%를 할인하여 팔았을 때 이익이 원가의 10% 이상이 되도록 하려 한다. 이때 정가의 최소값을 구하시오.`, `5500원`);
  if (type===2) return createProblem('2-1-4', diff, `집에서 역까지의 거리는 3km이다. 한 학생이 처음에는 시속 3km로 걷다가 중간부터 시속 4km로 뛰어 50분(= \\(\\frac{5}{6}\\)시간) 이내에 도착하고자 한다. 뛴 거리의 최소값을 구하시오.`, `1km`);
  if (type===3) return createProblem('2-1-4', diff, `농도 10%의 소금물 200g이 있다. 물만 증발시켜 소금물의 농도가 20% 이상이 되게 하려면 증발시켜야 할 물의 양의 범위를 구하시오.`, `100g 이상`);
  if (type===4) return createProblem('2-1-4', diff, `길이가 같은 긴 의자가 여러 개 있다. 한 의자에 4명씩 앉으면 5명이 서 있고, 5명씩 앉으면 의자 2개가 빈다. 이때 의자의 개수를 구하시오.`, `12개`);
  if (type===5) return createProblem('2-1-4', diff, `부등식 $ax+b < 0$의 해가 $x > 2$일 때, 부등식 $bx+a > 0$의 해를 구하시오.`, `$x < -\\dfrac{1}{2}$`);
  if (type===6) return createProblem('2-1-4', diff, `부등식 $\\begin{cases} x > 2 \\\\ x < a \\end{cases}$가 해를 가지지 않을 때, 상수 $a$의 범위를 구하시오.`, `$a \\le 2$`);
  if (type===7) return createProblem('2-1-4', diff, `집 앞 가게에서는 사과 한 개를 1000원에, 시장에서는 사과 한 개를 800원에 판다. 다만 시장에 갔다 오는 왕복 교통비는 1500원이다. 사과를 몇 개 이상 살 때 시장에 가는 것이 더 이득인지 부등식을 세워 구하시오.`, `8개 이상`);
  if (type===8) return createProblem('2-1-4', diff, `절댓값 부등식 $|x-1| < 3$을 해집합의 형태로 나타내시오.`, `-2 < x < 4`);
  if (type===9) return createProblem('2-1-4', diff, `윗변의 길이가 5, 높이가 6인 사다리꼴의 넓이가 40 이상이 되도록 하는 아랫변의 길이 $x$의 범위를 구하시오.`, `$x \\ge \\dfrac{25}{3}$`);
  if (type===10) return createProblem('2-1-4', diff, `일차부등식 $\\dfrac{x-a}{3} > 1$의 해가 $x > 5$가 되도록 하는 상수 $a$의 값을 구하시오.`, `2`);
  if (type===11) return createProblem('2-1-4', diff, `부등식 $3x-2 > x+4$의 해와 $2x+a < 5x$의 해가 서로 같을 때, 상수 $a$의 값을 구하시오.`, `-9`);
  if (type===12) return createProblem('2-1-4', diff, `부등식 $3(x-1) \\ge 5x-a$의 해 중 가장 큰 정수가 3일 때, 상수 $a$의 값을 구하시오.`, `9`);
  if (type===13) return createProblem('2-1-4', diff, `농도 10%의 소금물 300g과 농도 20%의 소금물을 섞어 농도가 16% 이하인 소금물을 만들고자 한다. 사용할 수 있는 20% 소금물의 최대 양을 구하시오.`, `450g`);
  if (type===14) return createProblem('2-1-4', diff, `빈 상자 1개의 무게는 2kg이고, 물건 1개의 무게는 300g이다. 상자 1개에 물건을 넣었을 때 전체 무게가 10kg 이하가 되도록 할 수 있는 물건의 최대 개수를 구하시오.`, `26개`);
  if (type===15) return createProblem('2-1-4', diff, `어떤 주차장의 요금은 30분까지 2000원이고, 이후에는 1분당 100원씩 추가된다. 5000원으로 주차할 수 있는 최대 시간을 분 단위로 구하시오.`, `60분`);
  if (type===16) return createProblem('2-1-4', diff, `한 상품의 정가는 $p$원이다. 이 상품을 10% 할인해서 팔아도 원가의 20% 이상의 이익을 얻으려면, 정가 $p$가 만족해야 하는 부등식을 세우고 $p$의 최솟값을 구하시오. (원가는 10000원으로 두어도 좋다.)`, `구조 서술형`);
  if (type===17) return createProblem('2-1-4', diff, `어떤 수 $x$에 대하여 $|2x-5| \\ge 7$일 때, 이를 두 개의 일차부등식으로 나타내고 해집합을 구하시오.`, `2x-5 \\ge 7$ 또는 $2x-5 \\le -7`);
  if (type===18) return createProblem('2-1-4', diff, `두 정수 $x, y$에 대하여 $x>y$이고 $x+y \\le 10$일 때, 가능한 $(x,y)$의 개수를 구하시오.`, `45쌍`);
  if (type===19) return createProblem('2-1-4', diff, `어떤 동전을 100개 이상 150개 이하 가지고 있다. 이 동전들을 8개씩 묶었을 때 남는 동전 수가 3개 이하가 되도록 하는 동전 개수의 범위를 부등식으로 나타내시오.`, `100 \\le x \\le 150, x \\equiv 0,1,2,3 \\pmod 8`);
  if (type===20) return createProblem('2-1-4', diff, `한 학생이 시험 세 번을 보았는데, 첫 번째와 두 번째 시험 점수는 각각 70점과 85점이다. 세 시험의 평균이 80점 이상이 되기 위한 세 번째 시험 점수 $x$의 범위를 부등식으로 세우고 구하시오.`, `$x \\ge 85$`);
  if (type===21) return createProblem('2-1-4', diff, `직사각형의 가로 길이는 $x$cm, 세로 길이는 8cm이다. 이 직사각형의 넓이가 100cm^2 이상이 되도록 하는 $x$의 범위를 구하시오.`, `$x \\ge 12.5$`);
  if (type===22) return createProblem('2-1-4', diff, `한 상자에 들어 있는 과자의 개수는 20개 이상 30개 이하이다. 이 상자를 $n$개 샀을 때, 과자의 총 개수가 200개 이상이 되도록 하는 $n$의 범위를 부등식으로 세우고, 가능한 최소 정수값 $n$을 구하시오.`, `n \\ge 7`);
  return createProblem('2-1-4', diff, `식당에서 점심값으로 6500원짜리 A세트와 8000원짜리 B세트만 판다. A세트는 최소 3개 이상 팔려야 하고, 하루 매출이 10만원 이상이 되도록 할 때, 판매 개수의 관계를 나타내는 부등식을 세우고 가능한 조합을 설명하시오.`, `구조 서술형`);
};
...
const funcLow = (): Problem => {
  ...
  if (type===1) return createProblem('2-1-5', diff, `일차함수 $y=2x+3$의 기울기를 구하시오.`, `2`);
  if (type===2) return createProblem('2-1-5', diff, `일차함수 $y=-x+5$의 $y$절편을 구하시오.`, `5`);
  if (type===3) return createProblem('2-1-5', diff, `함수 $f(x)=3x-1$일 때, $f(2)$의 값을 구하시오.`, `5`);
  if (type===4) return createProblem('2-1-5', diff, `직선 $y=3x$를 $y$축 방향으로 -2만큼 평행이동한 직선의 방정식을 구하시오.`, `$y=3x-2$`);
  if (type===5) return createProblem('2-1-5', diff, `점 $(1, a)$가 직선 $y=2x$ 위의 점일 때, $a$의 값을 구하시오.`, `2`);
  if (type===6) return createProblem('2-1-5', diff, `직선 $y = ax - 4$의 $x$절편이 2일 때, $a$의 값을 구하시오.`, `2`);
  ...
  if (type===18) return createProblem('2-1-5', diff, `직선 $y=3x+1$과 $y$축이 만나는 점의 좌표를 구하시오.`, `(0,1)`);
  if (type===19) return createProblem('2-1-5', diff, `한 직선의 그래프가 원점을 지나고 점 $(2,3)$을 지난다. 이 직선의 기울기를 구하시오.`, `\\dfrac{3}{2}`);
  if (type===20) return createProblem('2-1-5', diff, `직선 $y=ax$의 그래프가 점 $(4,6)$을 지날 때, $a$의 값을 구하시오.`, `\\dfrac{3}{2}`);
  if (type===21) return createProblem('2-1-5', diff, `함수 $y=x+2$에서 $x$가 1 증가할 때 $y$는 얼마만큼 증가하는지 구하시오.`, `1`);
  if (type===22) return createProblem('2-1-5', diff, `일차함수 $y=-2x+1$의 기울기와 $y$절편을 각각 구하시오.`, `기울기 -2, y절편 1`);
  return createProblem('2-1-5', diff, `다음 중 그래프가 원점을 지나는 식을 모두 고르시오.  ($y=2x+1, y=-x, y=3$)`, `$y=-x$`);
};
...
const funcMid = (): Problem => {
  ...
  if (type===1) return createProblem('2-1-5', diff, `두 점 $(1,3)$과 $(2,5)$를 지나는 직선의 방정식을 구하시오.`, `$y=2x+1$`);
  if (type===2) return createProblem('2-1-5', diff, `기울기가 3이고 점 $(1, 2)$를 지나는 직선의 방정식을 구하시오.`, `$y=3x-1$`);
  if (type===3) return createProblem('2-1-5', diff, `$x$절편이 3이고 $y$절편이 6인 직선의 방정식을 구하시오.`, `$y=-2x+6$`);
  ...
  if (type===16) return createProblem('2-1-5', diff, `두 직선 $y=-x+4$와 $y=2x-5$의 교점의 좌표를 구하시오.`, `(3,1)`);
  if (type===17) return createProblem('2-1-5', diff, `함수 $y=-\\dfrac{1}{2}x+1$의 그래프가 지나는 두 점을 예를 들어 두 개 쓰시오.`, `(0,1), (2,0) 등`);
...
const funcHigh = (): Problem => {
  ...
  if (type===1) return createProblem('2-1-5', diff, `세 직선 $y=x$, $y=-x+4$, $y=0$으로 둘러싸인 삼각형의 넓이를 구하시오.`, `4`);
  if (type===2) return createProblem('2-1-5', diff, `두 직선 $y=ax+2$와 $y=-x+b$가 $y$축 위의 한 점에서 서로 수직으로 만날 때, 교점과 상수 $b$의 값을 구하시오.`, `교점 (0,2), b=2`);
  if (type===3) return createProblem('2-1-5', diff, `사각형 $ABCD$의 꼭짓점이 $A(0,4)$, $B(4,4)$, $C(4,0)$, $D(0,0)$일 때, 넓이를 이등분하는 직선 $y=ax$에서 $a$의 값을 구하시오.`, `1`);
  if (type===4) return createProblem('2-1-5', diff, `100L의 물이 담긴 물통에서 2분마다 5L씩 물이 빠져나간다. $x$분 후 남아 있는 물의 양 $y$를 $x$에 대한 일차함수로 나타내시오.`, `$y=100-2.5x$`);
  if (type===5) return createProblem('2-1-5', diff, `일차함수 $ax+by-c=0$에서 $a>0, b<0, c>0$일 때, 그래프가 지나는 사분면을 모두 고르시오.`, `제1, 3, 4사분면`);
  if (type===6) return createProblem('2-1-5', diff, `두 일차함수 $y=-x+4$와 $y=x-a$의 교점의 $x$좌표가 양수가 되도록 하는 $a$의 범위를 구하시오.`, `$a > -4$`);
  if (type===7) return createProblem('2-1-5', diff, `엘리베이터가 1층에서 출발하여 2초마다 1층씩 올라갈 때, 출발 후 $x$초가 지났을 때 도달한 층수 $y$를 일차함수로 나타내시오.`, `$y=0.5x+1$`);
  if (type===8) return createProblem('2-1-5', diff, `직사각형 $ABCD$에서 $AB=4$, $BC=6$이고, 점 $P$는 변 $BC$ 위를 $B$에서 $C$ 방향으로 움직인다. $BP$의 길이를 $x$라고 할 때, 삼각형 $ABP$의 넓이 $y$를 $x$에 대한 일차함수로 나타내시오.`, `$y=2x$`);
  if (type===9) return createProblem('2-1-5', diff, `연립방정식이 해를 가지지 않을 때, 두 직선의 위치 관계를 서술하시오.`, `서로 평행하다.`);
  if (type===10) return createProblem('2-1-5', diff, `두 직선 $y=x+2$와 $y=-2x+8$, $x$축으로 둘러싸인 도형의 넓이를 구하시오.`, `9`);
  if (type===11) return createProblem('2-1-5', diff, `세 직선 $y=2x$, $y=-x+3$, $y=ax-1$이 삼각형을 이루지 않도록 하는 정수 $a$의 값을 모두 구하시오.`, `2, -1`);
  if (type===12) return createProblem('2-1-5', diff, `그래프 $|x| + |y| = 2$로 나타나는 도형의 넓이를 구하시오.`, `8`);
  if (type===13) return createProblem('2-1-5', diff, `A는 분속 100m로 먼저 출발하고, B는 10분 뒤에 분속 200m로 같은 방향으로 출발한다. 두 사람이 만나는 시각을 A가 출발한 때를 기준으로 구하시오.`, `20분 후`);
  if (type===14) return createProblem('2-1-5', diff, `직사각형의 꼭짓점이 $A(1,1)$, $B(3,1)$, $C(3,4)$, $D(1,4)$일 때, 직선 $y=ax$가 이 직사각형과 만나기 위한 $a$의 범위를 구하시오.`, `$\\dfrac{1}{3} \\le a \\le 4$`);
  if (type===15) return createProblem('2-1-5', diff, `어떤 상품의 가격은 시간에 따라 일차함수적으로 감소하여, 0시간일 때 10000원, 5시간 후에는 5000원이 되었다. 시간 $x$(시간) 후의 가격 $y$(원)를 일차함수로 나타내시오.`, `$y=-1000x+10000$`);
  if (type===16) return createProblem('2-1-5', diff, `두 직선 $y=2x+1$과 $y=-x+b$가 서로 수직으로 만날 때, $b$의 값을 구하시오.`, `-1`);
  if (type===17) return createProblem('2-1-5', diff, `일차함수 $y=ax+b$의 그래프가 점 $(1,2)$와 $(3,6)$을 지날 때, $a$와 $b$의 값을 구하시오.`, `a=2, b=0`);
  if (type===18) return createProblem('2-1-5', diff, `어떤 직선의 방정식은 $y=-\\dfrac{3}{2}x+6$이다. 이 직선의 그래프가 지나는 두 점의 좌표를 예로 들어 두 개 쓰시오.`, `(0,6), (4,0) 등`);
  if (type===19) return createProblem('2-1-5', diff, `함수 $y=ax+1$의 그래프가 제2, 4사분면을 지나지 않도록 하는 실수 $a$의 범위를 구하시오.`, `a>0`);
  if (type===20) return createProblem('2-1-5', diff, `두 직선 $y=x+1$과 $y=-x+5$로 둘러싸인 영역의 넓이를 구하시오.`, `8`);
  if (type===21) return createProblem('2-1-5', diff, `어떤 물체가 초속 5m로 직선 위를 움직이고 있다. 시간 $t$초 후의 위치를 $x$라 할 때, $x$를 $t$에 대한 일차함수로 나타내시오. (처음 위치는 원점이라고 가정)`, `$x=5t$`);
  if (type===22) return createProblem('2-1-5', diff, `용수철의 자연 길이는 20cm이고, 10g의 추를 매달 때마다 2cm씩 늘어난다. 추의 무게가 $x$g일 때 용수철의 길이 $y$를 일차함수로 나타내시오.`, `$y=20+0.2x$`);
  return createProblem('2-1-5', diff, `두 직선 $y=4x-1$과 $y=-2x+5$의 교점 좌표를 구하시오.`, `(1,3)`);
};
